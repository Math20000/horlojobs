const state = { jobs: [], feed: null, filter: 'all', query: '', source: '', sort: 'score', favorites: new Set(JSON.parse(localStorage.getItem('horlojobs-favorites') || '[]')), map: null, markers: null, detailMap: null };
...
function renderDetailMap(job) {
  const map = $('#detail-map'); const lat = Number(job.coordinates?.lat); const lon = Number(job.coordinates?.lon);
  if (!map || !window.L || !Number.isFinite(lat) || !Number.isFinite(lon)) return;
  state.detailMap = L.map(map, { attributionControl: false, zoomControl: false, dragging: false, touchZoom: false, doubleClickZoom: false, boxZoom: false, keyboard: false, scrollWheelZoom: false }).setView([lat, lon], 11);
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19 }).addTo(state.detailMap);
  L.marker([lat, lon], { icon: L.divIcon({ className: 'detail-map-marker-container', html: '<span class="detail-map-marker">⌖</span>', iconSize: [30, 30], iconAnchor: [15, 15] }) }).addTo(state.detailMap);
  setTimeout(() => state.detailMap?.invalidateSize(), 0);
}

function showDetails(job, openLetter = false) {
  clearDetailMap(); const detail = $('#job-detail'); const mailSubject = encodeURIComponent(`Candidature — ${job.title}`); const letter = job.coverLetter || {}; const letterHtml = letter.body ? `<details class="letter-box" ${openLetter ? 'open' : ''}><summary>Préparer la lettre pour ${escapeHtml(letter.targetCompany || job.company)}</summary>${letter.note ? `<p>${escapeHtml(letter.note)}</p>` : ''}<pre>${escapeHtml(letter.body)}</pre><button class="copy-letter" type="button">Copier la lettre</button></details>` : '';
  const coordinatesReady = Number.isFinite(Number(job.coordinates?.lat)) && Number.isFinite(Number(job.coordinates?.lon)); const detailMapHtml = coordinatesReady ? `<section class="detail-section detail-map-section"><h3>Localisation</h3><div id="detail-map" role="img" aria-label="Carte de localisation de l’offre"></div><p class="detail-map-note">Repère basé sur la ville ou le lieu public de l’annonce.</p></section>` : '';
  detail.innerHTML = `<h2 class="detail-title">${escapeHtml(job.title)}</h2><p class="detail-company">${escapeHtml(job.company)}</p><div class="detail-meta"><span>Score ${job.score}/100</span>${job.isNew ? '<span>Nouvelle offre</span>' : ''}${job.location ? `<span>⌖ ${escapeHtml(job.location)}, ${escapeHtml(job.countryLabel || '')}</span>` : ''}${job.contract ? `<span>${escapeHtml(job.contract)}</span>` : ''}</div>${detailMapHtml}<section class="detail-section"><h3>Salaire moyen estimé</h3><p><strong>${escapeHtml(job.salaryEstimate?.label || 'À estimer')}</strong>${job.salaryEstimate?.secondary ? `<br>${escapeHtml(job.salaryEstimate.secondary)}` : ''}<br>${escapeHtml(job.salaryEstimate?.net || '')}<br><small>${escapeHtml(job.salaryEstimate?.kind || '')}</small></p></section><section class="detail-section"><h3>Pourquoi cette offre correspond</h3><p>${escapeHtml(job.reason || 'Correspondance avec tes compétences horlogères.')}</p></section>${job.matchedSkills ? `<section class="detail-section"><h3>Compétences détectées</h3><p>${escapeHtml(job.matchedSkills)}</p></section>` : ''}${job.description ? `<section class="detail-section"><h3>Description</h3><p>${escapeHtml(job.description)}</p></section>` : ''}${letterHtml}<div class="detail-actions"><a href="${escapeHtml(job.url)}" target="_blank" rel="noopener">Ouvrir l’annonce</a><a href="mailto:?subject=${mailSubject}">Préparer l’e-mail</a></div>`;
  ...
  $('#job-dialog').showModal(); renderDetailMap(job);
}
